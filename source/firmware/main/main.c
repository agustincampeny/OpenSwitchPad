...
void send_buttons() {
    ...
    if(!paired) {
        esp_hid_device_send_report(ESP_HIDD_REPORT_TYPE_INTRDATA, 0xa1, sizeof(dummy), dummy);
        vTaskDelay(100);
    }
    else {
...
void app_main() {
    ...
    spi_init();
    xTaskCreatePinnedToCore(get_buttons, "gbuttons", 2048, NULL, 1, &ButtonsHandle, 1);
    //flash LED
    vTaskDelay(100);
    gpio_set_level(LED_GPIO, 0);
    vTaskDelay(100);
